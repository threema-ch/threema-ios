...
- (void)viewWillAppear:(BOOL)animated {
    ...
    switch ([Colors getTheme]) {
        ...
        case ColorThemeDarkWork:
            self.qrImageView.image = [qrCodeImage imageWithTint:[UIColor whiteColor]];
            break;
...
- (NSString *)tableView:(UITableView *)tableView titleForFooterInSection:(NSInteger)section {
    if (section == 0) {
        if (mdmSetup.disableSystemBackups) {
            ...
            if (self.phoneBackupSwitch.on) {
                return NSLocalizedString(@"backup_include", nil);
            } else {
                return NSLocalizedString(@"backup_exclude", nil);
            }
        }
    } else if (section == 1) {
        return NSLocalizedString(@"backup_footer", nil);
    }
...
- (NSString *)activityViewController:(UIActivityViewController *)activityViewController subjectForActivityType:(UIActivityType)activityType {
    NSString *subject = [NSString stringWithFormat:NSLocalizedString(@"backup_mail_subject", nil), [MyIdentityStore sharedMyIdentityStore].identity];
    return subject;
}
