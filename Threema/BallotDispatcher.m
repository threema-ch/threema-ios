...
+ (void)showViewControllerForBallot:(Ballot *)ballot onNavigationController:(UINavigationController*)navigationController {
    ...
    if ([ballot isClosed]) {
        ...
        [self pushController:viewController onNavigationController:navigationController];
    } else {
...
+ (void)pushController:(UIViewController*)viewController onNavigationController:(UINavigationController*)navigationController {
    if (SYSTEM_IS_IPAD) {
        ModalNavigationController *modalNav = [[ModalNavigationController alloc] initWithRootViewController:viewController];
        modalNav.showDoneButton = YES;
        modalNav.dismissOnTapOutside = YES;
