...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [self setTitle:NSLocalizedStringFromTable(@"ballots", @"Ballot", nil)];
...
-(NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section {
    if (section == 0) {
        return NSLocalizedStringFromTable(@"ballot_open_ballots", @"Ballot", nil);
    } else {
        return NSLocalizedStringFromTable(@"ballot_closed_ballots", @"Ballot", nil);
    }
}
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        [_entityManager performSyncBlockAndSafe:^{
            ...
            ChatViewController *chatViewController = [ChatViewControllerCache controllerForConversation:_conversation];
