...
#define IMAGE_COLOR [Colors fontNormal]
...
- (void)setColorForChoice:(UIColor *)color {
    ...
    [subViews enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj isKindOfClass:[UIImageView class]]) {
            ...
            imageView.image = [imageView.image imageWithTint:color];
        }
    }];
}
...
- (void)updateResultForChoice:(BallotChoice *)choice andParticipant:(NSString *)participant {
    ...
    if (result == nil) {
        ...
        self.accessibilityValue = NSLocalizedString(@"yes", nil);
    } else {
        UIImageView *minus = [self getMinus];
        [self addSubview: minus];
        self.accessibilityValue = NSLocalizedString(@"no", nil);
    }
}
