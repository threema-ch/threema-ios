...
@objc open class ChatContactInfoSystemMessageCell: UITableViewCell {
    ...
    @objc override public init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        if LicenseStore.requiresLicenseKey() {
            _msgBackground.backgroundColor = Colors.green()
        } else {
            _msgBackground.backgroundColor = Colors.workBlue()
        }
        ...
        _threemaTypeIcon.frame = CGRect.init(x: 20.0, y: _iconY, width: _iconSize, height: _iconSize)
        contentView.addSubview(_threemaTypeIcon)
        
        _msgText.frame = CGRect.init(x: 20.0, y: 21, width: contentView.frame.size.width - 40.0, height: 20.0)
...
extension ChatContactInfoSystemMessageCell {
    ...
    @objc open class func height(for message: BaseMessage!, forTableWidth tableWidth: CGFloat) -> CGFloat {
        ...
        let maxSize = CGSize.init(width: tableWidth - 26.0 - 8.0, height: CGFloat.greatestFiniteMagnitude)
    ...
    @objc override open func layoutSubviews() {
        ...
        if (textSize.height > _iconSize) {
            _msgBackground.frame = CGRect(x: backgroundX, y: _iconY - bgTopOffset, width: backgroundWidth, height: textSize.height + bgHeightMargin)
            _threemaTypeIcon.frame = CGRect.init(x: backgroundX + bgSideMargin, y: _iconY + (textSize.height / 2) - (_iconSize / 2), width: _iconSize, height: _iconSize)
            _msgText.frame = CGRect.init(x: backgroundX + bgSideMargin + _iconSize + bgSideMargin, y: _iconY, width: textSize.width, height: textSize.height)
        } else {
            _msgBackground.frame = CGRect(x: backgroundX, y: _iconY - bgTopOffset, width: backgroundWidth, height: _iconSize + bgHeightMargin)
            _threemaTypeIcon.frame = CGRect.init(x: backgroundX + bgSideMargin, y: _iconY, width: _iconSize, height: _iconSize)
            _msgText.frame = CGRect.init(x: backgroundX + bgSideMargin + _iconSize + bgSideMargin, y: _iconY + (_iconSize / 2) - (textSize.height / 2), width: textSize.width, height: textSize.height)
        }
    }
