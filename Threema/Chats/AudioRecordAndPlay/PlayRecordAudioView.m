...
- (void)setup {
    ...
    [_sendButton setTitle:[BundleUtil localizedStringForKey:@"send"] forState:UIControlStateNormal];
...
- (void)updateColors {
    _graphView.backgroundColor = Colors.backgroundAudioPlayer;
    _dataView.backgroundColor = Colors.backgroundAudioPlayer;
    _buttonView.backgroundColor = Colors.backgroundAudioPlayerButton;
    
    _horizontalDividerLine.backgroundColor = Colors.hairLine;
    _verticalDividerLine.backgroundColor = Colors.hairLine;
    
    _speedButton.backgroundColor = Colors.backgroundSpeedButton;
...
- (void)updateTimerFired {
    ...
    _durationLabel.text = [ThreemaUtilityObjC timeStringForSeconds:duration];
    _timeCursorLabel.text = [ThreemaUtilityObjC timeStringForSeconds:position];
    
    if (UIAccessibilityIsVoiceOverRunning()) {
        _durationLabel.accessibilityLabelBlock = ^NSString *() {
            return [ThreemaUtilityObjC accessibilityStringAtTime:duration withPrefix:@"duration"];
        };

        _timeCursorLabel.accessibilityLabelBlock = ^NSString *() {
            return [ThreemaUtilityObjC accessibilityStringAtTime:position withPrefix:@"current_position"];
        };
    }
}
