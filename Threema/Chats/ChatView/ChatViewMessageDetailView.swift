...
class ChatViewMessageDetailView: ThemedCodeModernGroupedTableViewController {
    ...
    private var messageCell: ChatViewBaseTableViewCell {
        guard let message = message else {
            fatalError("No message to show in details")
        }

        let cell = ChatViewCellProvider.cellType(for: message)
    ...
    init(messageID: NSManagedObjectID?) {
...
extension ChatViewMessageDetailView: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, shouldShowMenuForRowAt indexPath: IndexPath) -> Bool {
        ...
        (tableView.cellForRow(at: indexPath)?.detailTextLabel?.text) != nil && indexPath.section == 1 && indexPath
