...
- (void)setupActivityIndicator {
    _activityIndicator = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:Colors.objcActivityIndicatorViewStyle];
            ...
            if (messageObject.objectID == self.message.objectID) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (self.message.wasDeleted == NO) {
                        if ([keyPathCopy isEqualToString:@"progress"] || [keyPathCopy isEqualToString:@"sent"]) {
                            [self updateProgress];
                            [self updateResendButton];
                        } else if ([keyPathCopy isEqualToString:@"sendFailed"]) {
                            [self updateProgress];
