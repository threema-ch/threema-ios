...
- (NSString *)accessibilityLabelForContent {
    return [NSString stringWithFormat:@"%@, %d %@", [BundleUtil localizedStringForKey:@"video"], ((VideoMessageEntity*)self.message).duration.intValue, [BundleUtil localizedStringForKey:@"seconds"]];
}
...
- (void)setMessage:(BaseMessage *)newMessage {
    ...
    VideoMessageEntity *videoMessageEntity = (VideoMessageEntity*)newMessage;
    ...
    thumbnailView.image = videoMessageEntity.thumbnail.uiImage;// thumbnailWithPlayOverlay;
    
    if (videoMessageEntity.isOwn.boolValue) {
    ...
    int seconds = videoMessageEntity.duration.intValue;
    int minutes = (seconds / 60);
    seconds -= minutes * 60;
    durationLabel.text = [NSString stringWithFormat:@"%d:%02d", minutes, seconds];
    
    [downloadSizeLabel setText: [ThreemaUtilityObjC formatDataLength:videoMessageEntity.videoSize.floatValue]];
        ...
        @try {
            ...
            if (messageObject.objectID == self.message.objectID) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if ([keyPathCopy isEqualToString:@"video"]) {
                        [self updateDownloadSize];
                    }
                });
            }
...
- (void)updateDownloadSize {
    ...
    if (videoMessageEntity.video != nil) {
        ...
        downloadBackground.hidden = videoMessageEntity.videoBlobId != nil ? NO : YES;
        downloadSizeLabel.hidden = NO;
    }
}

- (void)messageTapped:(id)sender {
    [self.chatVc videoMessageTapped:(VideoMessageEntity*)self.message];
}

- (BOOL)canPerformAction:(SEL)action withSender:(id)sender {
    VideoMessageEntity *videoMessageEntity = (VideoMessageEntity*)self.message;
    if (action == @selector(resendMessage:) && videoMessageEntity.isOwn.boolValue && videoMessageEntity.sendFailed.boolValue) {
        return YES;
    } else if (action == @selector(deleteMessage:) && videoMessageEntity.isOwn.boolValue && videoMessageEntity.progress != nil) {
        ...
        return (videoMessageEntity.video != nil);  /* can only save downloaded videos */
...
- (BOOL)shouldHideBubbleBackground {
    VideoMessageEntity *videoMessageEntity = (VideoMessageEntity*)self.message;
    return (videoMessageEntity.thumbnail != nil);
}
        ...
        if (videoMessageEntity.video.data != nil) {
            ...
            UIContextMenuConfiguration *conf = [UIContextMenuConfiguration configurationWithIdentifier:indexPath previewProvider:^UIViewController * _Nullable{
                ...
                UIAction *action = [UIAction actionWithTitle:[BundleUtil localizedStringForKey:@"save"] image:copyImage identifier:nil handler:^(__kindof UIAction * _Nonnull action) {
                    ...
                    if (![videoMessageEntity.video.data writeToURL:tmpurl atomically:NO]) {
