...
- (void)executeAction {
    if (_mediaPickerType == MediaPickerChooseExisting) {
        ...
        if (self.chatViewController.chatBar) {
                ...
                if (![captions[i] isEqualToString:@""]) {
                    [MessageSender sendMessage:captions[i]
                                inConversation:self.chatViewController.conversation
                                    quickReply:false
                                     requestId:nil
                                  onCompletion:^(__unused BaseMessage *message) {
                        ;//nop
                    }];
                }
            }
...
- (void)blobMessageSender:(Old_BlobMessageSender*)blobMessageSender uploadFailedForMessage:(BaseMessage *)message error:(UploadError)error {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (isAlertShowing == NO) {
            isAlertShowing = YES;
            [UIAlertTemplate showAlertWithOwner:[[AppDelegate sharedAppDelegate] currentTopViewController] title:errorTitle message:errorMessage actionOk:^(__unused UIAlertAction * __nonnull alertAction){
                isAlertShowing = NO;
            }];
        }
    });
