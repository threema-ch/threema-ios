...
@interface Old_ChatBar () <OEMentionsHelperDelegate, HPGrowingTextViewDelegate, ConnectionStateDelegate>

@end

@implementation Old_ChatBar {
...
- (id)initWithFrame:(CGRect)frame conversation:(Conversation * _Nonnull)conversation
{
    ...
    if (self) {
        ...
        sendButtonText = [BundleUtil localizedStringForKey:@"send"];
...
- (void)updateColors {
    ...
    self.backgroundColor = Colors.backgroundChatBar;
    chatInput.backgroundColor = [UIColor clearColor];
    chatInput.textColor = Colors.text;
    ...
    chatInputBackgroundView.layer.borderColor = Colors.hairLine.CGColor;
...
- (void)updateHeight {
    ...
    if ([self.delegate respondsToSelector:@selector(old_chatBar:didChangeHeight:)]) {
        [self.delegate old_chatBar:self didChangeHeight:height];
    }
}
...
- (void)sendText {
    ...
    if ([self.delegate respondsToSelector:@selector(old_chatBar:didSendText:)]) {
        NSString *textToSend = chatInput.text;
        if (textToSend.length > 0 && quoteView.hidden == NO) {
            textToSend = [QuoteUtil generateText:textToSend quotedId:quoteView.quotedMessage.id];
        }
        
        [self.delegate old_chatBar: self didSendText: textToSend];
    }
}

- (void)addAction:(id)sender {
    [self.delegate old_chatBarDidPushAddButton:self];
}
...
- (void)chatBarTapped {
    ...
    if ([self.delegate respondsToSelector:@selector(old_chatBarTapped:)]) {
        [delegate old_chatBarTapped:self];
    }
}
...
- (BOOL)growingTextView:(HPGrowingTextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    ...
    if ([MessageSender sendTypingIndicatorWithConversation:conversation]) {
        ...
        if (!typing) {
            typing = YES;
            [delegate old_chatBarWillStartTyping:self];
...
- (void)handlePasteItem {
    // Allow pasting all items because we can reuse the item handling from the share extension
    ...
    if (!memoji) {
        if ([UIPasteboard generalPasteboard].itemProviders != nil) {
            [delegate old_chatBar:self didPasteItems:[UIPasteboard generalPasteboard].itemProviders];
            return;
        }
    } else {
        NSData *imageData = [[UIPasteboard generalPasteboard] dataForPasteboardType:(__bridge NSString *)kUTTypeImage];
        [delegate old_chatBar:self didSendImageData:imageData];
        return;
    }
...
- (void)handlePastedImage {
    NSData *imageData = [[UIPasteboard generalPasteboard] dataForPasteboardType:(__bridge NSString *)kUTTypeImage];
    if (imageData != nil) {
        [delegate old_chatBar:self didPasteImageData:imageData];
    } else {
        [delegate old_chatBar:self didPasteImageData:UIImageJPEGRepresentation([UIPasteboard generalPasteboard].image, 1.0)];
    }
}
...
- (void)showQuotedMessage {
    ...
    if ([self.delegate respondsToSelector:@selector(old_chatBarDidAddQuoteold_chatBarDidAddQuote)]) {
        [self.delegate old_chatBarDidAddQuote];
    }
