...
@objc public class RoundedRectLabel: UILabel {
    ...
    override public func draw(_ rect: CGRect) {
        ...
        let frameSize = CGSize(
            width: floor(frame.size.width - cornerRadius * 0.75 * 2 + 1),
            height: floor(frame.size.height + 1)
        )
        ...
        if textAlignment == .center {
            textRect = CGRect(
                ...
                y: (frame.size.height - textSize.height) / 2,
            ...
            textRect = CGRect(
                x: frame.size.width - textSize.width,
                y: (frame.size.height - textSize.height) / 2,
                width: textSize.width,
                height: textSize.height
            )
    ...
    func draw(roundedRect: CGRect, context: CGContext) -> CGRect {
        ...
        guard let fillColor = bgc?.cgColor else {
