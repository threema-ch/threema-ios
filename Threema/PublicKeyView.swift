...
final class PublicKeyView: UIView {
    ...
    private lazy var publicKeyLabel: CopyLabel = {
        ...
        if #available(iOS 13.0, *) {
            ...
            label.font = UIFont.monospacedDigitSystemFont(ofSize: preferredFont.pointSize, weight: .regular)
        }
    ...
    @objc public func updateColors() {
        ...
        identityTextView.textColor = Colors.fontNormal()
        identityTextView.backgroundColor = .clear

        publicKeyLabel.textColor = Colors.fontNormal()

        hairline.backgroundColor = Colors.hairline()
        
        okButton.tintColor = Colors.main()
        okButton.setTitleColor(Colors.main(), for: .normal)
        okButton.backgroundColor = Colors.background()
    }
    ...
    @objc func touchButtonDown() {
        okButton.backgroundColor = Colors.backgroundSelectedDark()
    }
}
...
extension UILabel {
    func addPublicKeyFormat(kernValue: Double = 6) {
        ...
        if #available(iOS 13.0, *) {
            attributedString.addAttribute(NSAttributedString.Key.font, value: UIFont.monospacedSystemFont(ofSize: preferredFont.pointSize, weight: .regular), range: range)
        } else {
            attributedString.addAttribute(NSAttributedString.Key.font, value: UIFont.monospacedDigitSystemFont(ofSize: preferredFont.pointSize, weight: .regular), range: range)
        }
