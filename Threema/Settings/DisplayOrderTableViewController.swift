...
class DisplayOrderTableViewController: ThemedTableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.textLabel?.text = BundleUtil.localizedString(forKey: orderKey)
        
        if (UserSettings.shared().displayOrderFirstName && indexPath.row == 0) ||
            (!UserSettings.shared().displayOrderFirstName && indexPath.row == 1) {
