...
@objc public class URLSenderItemCreator : NSObject {
    ...
    @objc public static func getSenderItem(for url : URL, maxSize : String?) -> URLSenderItem? {
        ...
        if UTIConverter.conforms(toMovieType: uti) {
            ...
            if (maxSize != nil) {
                creator = ImageURLSenderItemCreator(with: maxSize!, forceSize: false)
            } else {
    ...
    @objc public static func getSenderItem(for url : URL) -> URLSenderItem? {
        return URLSenderItemCreator.getSenderItem(for: url, maxSize: nil)
    }
