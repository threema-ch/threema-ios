...
final class MWPhotoBrowserWrapper: NSObject, MWPhotoBrowserDelegate, MWVideoDelegate, MWFileDelegate,
    ...
    func prepareMedia() {
        ...
        let imageMessages = entityManager.entityFetcher.imageMessages(for: conversation) ?? [ImageMessage]()
        let videoMessages = entityManager.entityFetcher.videoMessages(for: conversation) ?? [VideoMessage]()
        ...
        let fileMessages = entityManager.entityFetcher.fileMessagesForPhotoBrowser(
            for: conversation,
            filteredMimeTypes: filters
        ) ?? []
