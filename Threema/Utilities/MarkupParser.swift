...
class MarkupParser {
    ...
    private static let URL_BOUNDARY_PATTERN = "[a-zA-Z0-9\\-.äöü_~:/?#\\[\\]@!$&'()*+,;=%]"
        ...
        parsedWithMentionMarkups
            .enumerateAttributes(in: NSRange(
                ...
                if attributes[NSAttributedString.Key.tokenType] as? MarkupParser.TokenType == TokenType.mention {
                    if let contact = attributes[NSAttributedString.Key.contact] as? Contact {
                        parsedWithMentionMarkups.replaceCharacters(in: range, with: "@[\(contact.identity)]")
                    }
                    else if let mentionAsString = attributes[NSAttributedString.Key.contact] as? String,
                            mentionAsString == MarkupParser.MENTION_ALL {
...
extension MarkupParser {
    ...
    private func isURLStart(text: String, position: Int, textCount: Int) -> Bool {
        ...
        guard let result = urlStartPattern?.matches(
            in: text,
            options: NSRegularExpression.MatchingOptions(rawValue: 0),
            range: NSRange(location: position, length: textCount - position)
        ) else {
            return false
        }
        for res in result {
    ...
    private func isMention(text: String, position: Int, textCount: Int) -> Bool {
        ...
        guard let result = mentionPattern?.matches(
            in: text,
            options: NSRegularExpression.MatchingOptions(rawValue: 0),
            range: NSRange(location: position, length: textCount - position)
        ) else {
                            ...
                            if var attributes = attributedItem.kind.attributes(font: font) {
                                ...
                                case .mention:
                                    ...
                                    if let contact = businessInjector.contactStore.contact(for: threemaID) {
                                        attributes.updateValue(contact, forKey: NSAttributedString.Key.contact)
                                    }
                                    else if threemaID == MarkupParser.MENTION_ALL {
                                        attributes.updateValue(
                                            MarkupParser.MENTION_ALL,
