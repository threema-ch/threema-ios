...
+ (NSString*)getClientVersion {
    /* Format: 1.0b234;I;de/CH;iPhone7,2;9.3 */
    NSString *appAndBuildVersion = [Utils appAndBuildVersion];
    
    struct utsname systemInfo;
    uname(&systemInfo);
    NSString *machine = [NSString stringWithCString:systemInfo.machine encoding:NSUTF8StringEncoding];
    
    NSString *clientVersion = [NSString stringWithFormat:@"%@;I;%@/%@;%@;%@", appAndBuildVersion,
            [[NSLocale currentLocale] objectForKey:NSLocaleLanguageCode],
            [[NSLocale currentLocale] objectForKey:NSLocaleCountryCode],
            machine, [[UIDevice currentDevice] systemVersion]];
...
+ (NSString *)appAndBuildVersion {
    ...
    NSString *suffix = [mainBundle objectForInfoDictionaryKey: @"ThreemaVersionSuffix"];
    if (suffix != nil)
        version = [version stringByAppendingString:suffix];
...
+ (UIView *)view:(UIView *)view getSuperviewOfKind:(Class)class {
    ...
    while (tmpView.superview) {
        if ([tmpView isKindOfClass:class]) {
            return tmpView;
        }
...
+ (BOOL)hideThreemaTypeIconForContact:(Contact *)contact {
    if (contact.isEchoEcho || contact.isGatewayId) {
        return YES;
    }
