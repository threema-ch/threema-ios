...
class WebProfileResponse: WebAbstractMessage {
    ...
    init(requestId: String?) {
        ...
        if MyIdentityStore.shared().pushFromName != nil {
            publicNickname = MyIdentityStore.shared().pushFromName
        } else {
            publicNickname = MyIdentityStore.shared().identity
        }
        
        if let profilePicture = MyIdentityStore.shared().profilePicture,
            profilePicture["ProfilePicture"] != nil {
            
            avatar = profilePicture["ProfilePicture"] as? Data
        }
        
        var tmpData:[AnyHashable:Any?] = ["identity": identity, "publicKey": publicKey, "publicNickname": publicNickname]
        ...
        super.init(messageType: "response", messageSubType: "profile", requestId: nil, ack: tmpAck, args: nil, data: tmpData)
    }
}
