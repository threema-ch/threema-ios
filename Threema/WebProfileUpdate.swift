...
class WebProfileUpdate: WebAbstractMessage {
    ...
    init(nicknameChanged: Bool, newNickname: String?, newAvatar: Data?, deleteAvatar: Bool) {
        ...
        if newAvatar != nil {
            let avatarImage = UIImage.init(data: newAvatar!)
            avatar = avatarImage!.jpegData(compressionQuality:CGFloat(kWebClientAvatarHiResQuality))!
