...
public class CallsignalingProtocol {
    ...
    public enum ThreemaVideoCallQualityProfiles: Int {
        ...
        public func qualityProfile() -> ThreemaVideoCallQualityProfile {
            switch self {
            case .max:
                return ThreemaVideoCallQualityProfile.init(bitrate: 4000, maxResolution: CGSize(width: 1920, height: 1080), maxFps: 25, profile: .max)
            case .high:
                return ThreemaVideoCallQualityProfile.init(bitrate: 2000, maxResolution: CGSize(width: 1280, height: 720), maxFps: 25, profile: .high)
            case .low:
                return ThreemaVideoCallQualityProfile.init(bitrate: 400, maxResolution: CGSize(width: 960, height: 540), maxFps: 20, profile: .low)
            }
        }
...
extension CallsignalingProtocol {
    ...
    public static func encodeVideoQuality(_ profile: ThreemaVideoCallQualityProfiles) -> Data? {
        ...
        videoQualityProfile.profile = Callsignaling_VideoQualityProfile.QualityProfile(rawValue: threemaVideoCallQualityProfile.profile!.rawValue)!
    ...
    public static func decodeThreemaVideoCallSignalingMessage(_ data: Data) -> ThreemaVideoCallSignalingMessage {
        ...
        case .captureStateChange(envelop?.captureStateChange):
            threemaVideoCallCaputreState = ThreemaVideoCallCaputreState.init(device: ThreemaVideoCallCaputreDevice(rawValue: envelop!.captureStateChange.device.rawValue)!, state: ThreemaVideoCallMode(rawValue: envelop!.captureStateChange.state.rawValue)!)
        case .videoQualityProfile(envelop?.videoQualityProfile):
            threemaVideoCallQualityProfile = ThreemaVideoCallQualityProfile.init(bitrate: envelop!.videoQualityProfile.maxBitrateKbps, maxResolution: CGSize(width: Int(envelop!.videoQualityProfile.maxResolution.width), height: Int(envelop!.videoQualityProfile.maxResolution.height)), maxFps: envelop!.videoQualityProfile.maxFps, profile: ThreemaVideoCallQualityProfiles(rawValue: envelop!.videoQualityProfile.profile.rawValue)!)
        default: break
        }
        
        return ThreemaVideoCallSignalingMessage(videoQualityProfile: threemaVideoCallQualityProfile, captureStateChange: threemaVideoCallCaputreState)
    }
}
...
extension CallsignalingProtocol {
    ...
    public static func currentThreemaVideoCallQualitySettingTitle() -> String {
        return threemaVideoCallQualitySettingTitle(for: UserSettings.shared().threemaVideoCallQualitySetting)
    }
    ...
    public static func threemaVideoCallQualitySettingTitle(for setting: ThreemaVideoCallQualitySetting) -> String {
        ...
        case ThreemaVideoCallQualitySettingAuto:
            return NSLocalizedString("settings_threema_calls_video_quality_profile_auto", comment: "")
        case ThreemaVideoCallQualitySettingMaximumQuality:
            return NSLocalizedString("settings_threema_calls_video_quality_profile_max", comment: "")
        case ThreemaVideoCallQualitySettingLowDataConsumption:
            return NSLocalizedString("settings_threema_calls_video_quality_profile_low", comment: "")
        default:
            return "Unknown"
        }
    }
    ...
    public static func threemaVideoCallQualitySettingSubtitle(for setting: ThreemaVideoCallQualitySetting) -> String {
        switch setting {
        case ThreemaVideoCallQualitySettingAuto:
            return NSLocalizedString("settings_threema_calls_video_quality_profile_auto_description", comment: "")
        case ThreemaVideoCallQualitySettingMaximumQuality:
            return NSLocalizedString("settings_threema_calls_video_quality_profile_max_description", comment: "")
        case ThreemaVideoCallQualitySettingLowDataConsumption:
            return NSLocalizedString("settings_threema_calls_video_quality_profile_low_description", comment: "")
        default:
            return "Unknown"
        }
    }
    ...
    public static func isThreemaVideoCallQualitySettingAuto() -> Bool {
        return UserSettings.shared()?.threemaVideoCallQualitySetting == ThreemaVideoCallQualitySettingAuto
    }
    
    public static func threemaVideoCallQualitySettingSelected(for setting: ThreemaVideoCallQualitySetting) -> Bool {
        return UserSettings.shared()?.threemaVideoCallQualitySetting == setting
    }
    ...
    public static func findCommonProfile(remoteProfile: ThreemaVideoCallQualityProfile?, networkIsRelayed: Bool, _ localProfile: ThreemaVideoCallQualityProfile? = nil) -> ThreemaVideoCallQualityProfile {
        ...
        if let foundProfile = remoteProfile?.profile {
            if foundProfile == .low || localQualityProfile.profile == .low {
                ...
                return networkIsRelayed ? ThreemaVideoCallQualityProfiles.high.qualityProfile() : ThreemaVideoCallQualityProfiles.max.qualityProfile()
            }
        }
        
        // Unknown profile
        let maxBitrateKbps = max(min(localQualityProfile.bitrate, remoteProfile!.bitrate), minBitrate)
        let maxResolutionWidth = max(min(localQualityProfile.maxResolution.width, remoteProfile!.maxResolution.width), minResolutionWidth)
        let maxResolutionHeight = max(min(localQualityProfile.maxResolution.height, remoteProfile!.maxResolution.height), minResolutionHeight)
        let maxFps = max(min(localQualityProfile.maxFps, remoteProfile!.maxFps), minFps)
        return ThreemaVideoCallQualityProfile.init(bitrate: maxBitrateKbps, maxResolution: CGSize(width: maxResolutionWidth, height: maxResolutionHeight), maxFps: maxFps, profile: remoteProfile!.profile)
    }
    
    public static func printDebugQualityProfiles(remoteProfile: ThreemaVideoCallQualityProfile?, networkIsRelayed: Bool) -> String {
        return "\(printLocalQualityProfile())\n\(printPeerQualityProfile(remoteProfile: remoteProfile))\n\(printCommonQualityProfile(remoteProfile: remoteProfile, networkIsRelayed: networkIsRelayed))"
    }
    ...
    private static func printCommonQualityProfile(remoteProfile: ThreemaVideoCallQualityProfile?, networkIsRelayed: Bool) -> String {
