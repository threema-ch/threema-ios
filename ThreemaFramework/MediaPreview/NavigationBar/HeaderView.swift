...
class HeaderView : UIView {
    ...
    init(for mediaPreviewItems : [MediaPreviewItem], frame : CGRect, tapAction : (() -> ())? = nil) {
        ...
        self.configureView(titleViewSize: frame)
        self.updateTitleLabel(mediaPreviewItems: mediaPreviewItems)
        self.rotate(landscape: UIDevice.current.orientation.isLandscape, newWidth: frame.width)
    ...
    func rotate(landscape : Bool, newWidth : CGFloat) {
        ...
        stackView.frame = CGRect(x: stackView.frame.minX,
    ...
    func configureView(titleViewSize : CGRect) {
        ...
        if tapAction != nil {
            ...
            attributedTitleString.addAttribute(NSAttributedString.Key.foregroundColor, value: Colors.main()!, range: range)
            
            let arrowAttach = NSTextAttachment()
            arrowAttach.image = BundleUtil.imageNamed("ArrowNext")?.withTint(Colors.main())
        ...
        // description.
        if stackView.frame.height > self.frame.height {
            stackView.frame = CGRect(x: stackView.frame.minX,
                                     y: stackView.frame.minY,
                                     width: stackView.frame.width,
    ...
    public func updateTitleLabel(mediaPreviewItems : [MediaPreviewItem]) {
        ...
        if mediaPreviewItems.count > 1 {
            tmpTitle = String(format: BundleUtil.localizedString(forKey:"multiple_media_items"), mediaPreviewItems.count)
        } else {
        ...
        viewButton.accessibilityLabel = title + BundleUtil.localizedString(forKey: "tapForOptions")
    }
}
