...
class ProgressViewHandler {
    ...
    func initProgress() {
        DispatchQueue.main.async(execute: {
            ...
            if hud.progressObject == nil {
                ...
                hud.label.text = String(format: BundleUtil.localizedString(forKey:"processing_items_progress"), po.completedUnitCount / 100, po.totalUnitCount / 100)
            }
        })
    }
    ...
    func incrementItemProgress(_ increment : Int64) {
        DispatchQueue.main.async(execute: {
            ...
            hud.label.text = String(format: BundleUtil.localizedString(forKey:"processing_items_progress"), completed, total)
    ...
    func hideHud(delayed : Bool = false, completion : @escaping () -> ()) {
