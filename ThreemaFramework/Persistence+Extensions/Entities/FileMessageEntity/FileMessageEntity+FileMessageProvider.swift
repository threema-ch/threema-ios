...
extension FileMessageEntity: VideoMessage, VoiceMessage {
    ...
    public func temporaryBlobDataURL() -> URL? {
        ...
        let url = fileUtility.appTemporaryUnencryptedDirectory.appendingPathComponent("\(filename).\(ext)")
        
        guard fileUtility.fileExists(at: url) || fileUtility.write(contents: data, to: url) else {
            DDLogWarn("Writing blob data to temporary file failed.")
            return nil
        }
