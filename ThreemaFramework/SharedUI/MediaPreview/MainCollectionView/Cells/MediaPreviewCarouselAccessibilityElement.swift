...
final class MediaPreviewCarouselAccessibilityElement: UIAccessibilityElement {
    ...
    override var accessibilityValue: String? {
        get {
            ...
            guard let description = delegate.mediaData[index.item].getAccessibilityDescription() else {
                return text
            }
    ...
    func accessibilityScrollCollectionView(forwards: Bool) -> Bool {
        ...
        if forwards {
            ...
            let newIndex = IndexPath(item: index.item + 1, section: 0)
            delegate.shouldScrollTo(indexPath: newIndex)
            currentMediaItem = newIndex
        }
        else {
            if index.item - 1 < 0 {
                return false
