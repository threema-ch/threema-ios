...
final class MainCollectionViewController: NSObject, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    private func handleImageItem(item: ImagePreviewItem, cell: ImagePreviewCollectionViewCell) {
        item.item.done { imageData in
            let mimeType = UTIConverter.mimeType(fromUTI: item.uti ?? "") ?? "application/octet-stream"
            let isGifMimeType = UTIConverter.isGifMimeType(mimeType)
            cell.updateImageTo(
                data: imageData,
                isGIF: isGifMimeType
            )
