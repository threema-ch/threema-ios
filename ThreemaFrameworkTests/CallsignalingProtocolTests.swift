...
final class CallsignalingProtocolTests: XCTestCase {
    ...
    func testSerializeProfile() throws {
        ...
        if envelop.content == .videoQualityProfile(envelop.videoQualityProfile) {
    ...
    func testFindCommonProfileLow() throws {
        ...
        for pair in pairs {
            let common = CallsignalingProtocol.findCommonProfile(
                remoteProfile: pair.a.qualityProfile(),
                networkIsRelayed: false,
                pair.b.qualityProfile()
            )
    ...
    func testFindCommonProfileMax() throws {
        ...
        let commonNonRelayed = CallsignalingProtocol.findCommonProfile(
            remoteProfile: a.qualityProfile(),
            networkIsRelayed: false,
            b.qualityProfile()
        )
        let commonRelayed = CallsignalingProtocol.findCommonProfile(
            remoteProfile: a.qualityProfile(),
            networkIsRelayed: true,
            b.qualityProfile()
        )
    ...
    func testFindCommonProfileRawValues() throws {
        let a = CallsignalingProtocol.ThreemaVideoCallQualityProfile(
            bitrate: 600,
            maxResolution: CGSize(width: 2000, height: 700),
            maxFps: 23,
            profile: CallsignalingProtocol.ThreemaVideoCallQualityProfiles(rawValue: 1234) ?? nil
        )
    ...
    func testFindCommonProfileRawValuesWithClamping() throws {
        let a = CallsignalingProtocol.ThreemaVideoCallQualityProfile(
            bitrate: 1,
            maxResolution: CGSize(width: 1, height: 1),
            maxFps: 1,
            profile: CallsignalingProtocol.ThreemaVideoCallQualityProfiles(rawValue: 1234) ?? nil
        )
        let b: CallsignalingProtocol.ThreemaVideoCallQualityProfiles = .high
