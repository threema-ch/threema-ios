...
final class ForwardSecurityRefreshStepsTests: XCTestCase {
    ...
    func testBasicRun() async throws {
        ...
        let fsRefreshSteps = ForwardSecurityRefreshSteps(backgroundBusinessInjector: businessInjectorMock)
        
        // The order of the identities is uses below for validation
        await fsRefreshSteps.run(for: [
            noSessionIdentity,
        ...
        let nonCommittedSessionInitMessage = try XCTUnwrap(nonCommittedSessionMessage.data as? ForwardSecurityDataInit)
