...
/// 2. Let `outer-messages` be an empty list.
/// 3. Lookup a session with `receiver` and let `session` be the result.
...
///       encrypted and encoded result to `outer-messages` with type `0xa0`.
...
///    `outer-messages` with type `0xa0`.
/// 7. Return `outer-messages` and an `fs-commit-fn` function that sets the
