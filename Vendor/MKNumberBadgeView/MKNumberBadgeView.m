...
- (void)initState;
{
	...
	self.opaque = NO;
	self.pad = 2;
	self.font = self.iOS7 ? [UIFont systemFontOfSize:16] : [UIFont boldSystemFontOfSize:16];
	self.shadow = YES;
...
- (void)drawRect:(CGRect)rect 
{
    ...
    if (self.value == 0) {
        ...
        numberString = [NSString stringWithFormat:@"%lu",(unsigned long)self.value];
    }
	...
	CGContextAddPath( curContext, badgePath );
	CGContextClosePath( curContext );
	CGContextDrawPath( curContext, iOS7 ? kCGPathFill : kCGPathFillStroke );
	...
	CGPoint textPt = CGPointMake( ctm.x + (badgeRect.size.width/2) - (numberSize.width / 1.75) , ctm.y + (badgeRect.size.height / 2) - (numberSize.height / 1.75));
