...
- (void)updateVisibleItemViewsWithPage:(NSUInteger)page
{
    ...
    if (numberOfPages > 1 && page != 0) {
        /* Custom */
        QBPopupMenuPagenatorView *leftPagenatorView = [[[self class] pagenatorViewClass] leftPagenatorViewWithTarget:self action:@selector(showPreviousPage) accessibilityLabel:_previousPageAccessibilityLabel];
    ...
    if (numberOfPages > 1 && page != numberOfPages - 1) {
        /* Custom */
        QBPopupMenuPagenatorView *rightPagenatorView = [[[self class] pagenatorViewClass] rightPagenatorViewWithTarget:self action:@selector(showNextPage) accessibilityLabel:_nextPageAccessibilityLabel];
...
- (CGMutablePathRef)leftSeparatorPathInRect:(CGRect)rect
{
    CGMutablePathRef path = CGPathCreateMutable();
    CGPathMoveToPoint(path, NULL, rect.origin.x, rect.origin.y);
    CGPathAddLineToPoint(path, NULL, rect.origin.x, rect.size.height);
    CGPathAddLineToPoint(path, NULL, rect.origin.x - rect.size.width, rect.size.height);
    CGPathAddLineToPoint(path, NULL, rect.origin.x - rect.size.width, rect.origin.y);
    CGPathCloseSubpath(path);
...
- (CGMutablePathRef)rightSeparatorPathInRect:(CGRect)rect
{
    CGMutablePathRef path = CGPathCreateMutable();
    CGPathMoveToPoint(path, NULL, rect.origin.x, rect.origin.y);
    CGPathAddLineToPoint(path, NULL, rect.origin.x, rect.size.height);
...
- (void)drawBodyInRect:(CGRect)rect firstItem:(BOOL)firstItem lastItem:(BOOL)lastItem highlighted:(BOOL)highlighted
{
    ...
    // Draw separator
    if (!firstItem) {
        [self drawLeftSeparatorInRect:CGRectMake(rect.origin.x, rect.origin.y + 1, 0.5, rect.size.height - 2)];
    }
    if (!lastItem) {
        [self drawRightSeparatorInRect:CGRectMake(rect.origin.x + rect.size.width - 1, rect.origin.y + 1, 0.5, rect.size.height - 2)];
    }
}

- (void)drawRightSeparatorInRect:(CGRect)rect
{
    CGContextRef context = UIGraphicsGetCurrentContext();
...
- (void)drawLeftSeparatorInRect:(CGRect)rect
{
    CGContextRef context = UIGraphicsGetCurrentContext();
